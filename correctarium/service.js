const calkSumAndTime = (requestJson) => {
  const request = parseRequest(requestJson);
  const price = getPrice(
    request.languageCoef,
    request.mimetypeCoef,
    request.count
  );
  const time = getTime(
    request.amountLetters,
    request.mimetypeCoef,
    request.count
  );
...
const getPrice = (languageCoef, mimetypeCoef, count) => {
  const minCount = 1000;
...
const getTime = (amountLetters, mimetypeCoef, count) => {
  const minTime = 60;
  const startTime = 30;
...
const parseRequest = (requestJson) => {
...
const prepareResponse = (price, time, deadline) => {
